...
import pdb
import os;
script_base_name = "several_polymer_y";
...
if flag_bond_1:
  ...
  KBT = units['KBT']; ell = 5.0; K = 2*KBT/(ell*ell); r0 = 20;
  ...
  print(f"I0 = {I0}, atom_name_list = {atom_name_list}")
  for i in range(num_polymers):
      atom_id = atom_id_list[i]; nn = len(atom_id);
      bonds = np.zeros((nn,2),dtype=int); 
      bond_id = np.zeros(nn, dtype=int)
      print(f"bonds.shape = {bonds.shape}, nn = {nn}")
      print(f"atom_id = {atom_id}, len(atom_id) = {len(atom_id)}")

      for j in range(0,nn):
          j1 = atom_id[j % nn]; j2 = atom_id[(j + 1)%nn];  # base 1 indexing, closed loop
          bonds[j,0] = j1; bonds[j,1] = j2;
          bond_id[j] = I_id;
          I_id += 1;  
...
compute totalPolymerStress polymers reduce sum c_polymerStress[4] c_polymerStress[5] c_polymerStress[6]

variable xzStress equal c_totalPolymerStress[1]

# changed to average the stress
#variable avg_shear_stress equal c_avgStress[3]/atoms
...
fix printPolymerStress all print 100 "${xzStress}" file polymer_xz_stress_output.txt screen no
...
dump dmp_stress polymers custom ${dumpfreq} stress_output.txt id type c_polymerStress[4] c_polymerStress[5] c_polymerStress[6]
dump_modify dmp_stress pad 8 #modify
